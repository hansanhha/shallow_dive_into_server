...
클러스터형 인덱스: InnoDB 스토리지 엔진에서 기본 키를 기준으로 테이블의 행 데이터를 물리적으로 정렬하여 저장하는 인덱스

클러스터형 인덱스에선 테이블의 데이터 자체가 **기본 키**를 기준으로 정렬되서 저장됨

테이블의 기본 키
- 클러스터형 인덱스를 형성
- 이 인덱스의 리프 노드에서 실제 테이블의 행 데이터 자체를 포함

보조 인덱스
- 기본 키가 아닌 다른 컬럼을 기반으로 생성됨
- 이 인덱스의 리프 노드는 기본 키 값을 가르킴

### 보조 인덱스와 클러스터형 인덱스를 통한 탐색 과정
